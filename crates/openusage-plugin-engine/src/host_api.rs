...
const WHITELISTED_ENV_VARS: [&str; 7] = [
    "CODEX_HOME",
    "GROK_HOME",
    "ZAI_API_KEY",
    "GLM_API_KEY",
...
fn keychain_write(service: &str, value: &str) -> Result<(), String> {
    if cfg!(target_os = "macos") {
        ...
        let output = std::process::Command::new("secret-tool")
            ...
            .map_err(|e| format!("secret-tool store failed: {}. Install libsecret-tools.", e))?;
    ...
    keychain_obj.set(
        ...
        Function::new(
            ...
            move |ctx_inner: Ctx<'_>, service: String| -> rquickjs::Result<String> {
                keychain_read(&service).map_err(|e| Exception::throw_message(&ctx_inner, &e))
            },
        )?,
    )?;
