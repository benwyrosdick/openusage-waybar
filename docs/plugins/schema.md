...
```
openusage-waybar invocation (Waybar exec / cron / interval)
       |
For each selected plugin:
  ...
  -> Parse returned `{ lines: MetricLine[] }`
       |
Aggregate `PluginOutput[]`
       |
Render Waybar JSON (text + Pango tooltip)
```

Key points:

- Each probe runs in an **isolated QuickJS runtime** (no shared state between plugins or calls)
- Plugins are **synchronous or Promise-based** (unresolved promises timeout)
- The CLI runs once per Waybar refresh (controlled by Waybar's `interval` or signal)
...
At runtime, plugins are discovered in `$OPENUSAGE_PLUGINS_DIR`, then `~/.local/share/openusage/plugins/`, then `~/.config/openusage/plugins/`, falling back to `./plugins/` for development.
...
| `primaryOrder` | number  | No       | Lower number = higher priority for primary metric selection |

- `"overview"` - shown on both Overview tab and plugin detail pages
- `"detail"` - shown only on plugin detail pages

### Primary Progress (Status Bar Headline)

Plugins can optionally mark one or more progress lines with `primaryOrder` (a positive integer). The lowest `primaryOrder` value identifies the headline metric used in the Waybar status text. Higher-numbered candidates are tried in order if the lower-numbered ones aren't present in the probe output.

Rules:
- Only `type: "progress"` lines can be primary (the field is ignored on other types)
- The candidate with the lowest `primaryOrder` that the probe actually returned wins
- If no `primaryOrder` line is returned, the first progress line is used as a fallback
...
```json
{
  "lines": [
    ...
    { "type": "progress", "label": "Extra", "scope": "detail", "primaryOrder": 2 },
